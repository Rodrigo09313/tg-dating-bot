import { handleIncomingPhoto } from "./bot/photoUpload.js";
// src/index.ts
      ...
      try {
        ...
        switch (state) {
          ...
          case "reg_city_text":
            ...
            return;
          case "reg_about":
            return; // обрабатывается твоими колбэками/отдельным хендлером
          ...
          case "reg_photo_upload_preview":
            if (msg.photo?.length) {
              await handleIncomingPhoto(bot, msg, fresh);
            } else {
              await sendScreen(bot, chatId, fresh, { text: "Отправьте фотографию или используйте кнопки на экране." });
            }
            return;
          case "reg_preview":
          ...
          // режимы редактирования профиля (оставлены как были)
          case "edit_about":
            if (msg.text) {
          ...
          case "edit_photo_upload":
            if (msg.photo?.length) await handleIncomingPhoto(bot, msg, fresh);
            return;
          case "edit_photo":
            if (msg.photo?.length) {
